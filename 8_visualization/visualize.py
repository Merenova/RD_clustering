...
def extract_attributions(graph, first_tokens: np.ndarray, logger) -> np.ndarray:
    """Extract attribution vectors from graph for each sample.

    Replicates the logic from cluster.py to extract a_n for each continuation.

    Args:
        graph: circuit_tracer Graph object
        first_tokens: Array of first token IDs per sample
        logger: Logger

    Returns:
        attributions: np.ndarray of shape (n_samples, n_attribution_nodes)
    """
    ...
    for s_n in first_tokens:
        ...
        # Ensure no NaNs/Infs
        attribution_vector = np.nan_to_num(attribution_vector, nan=0.0, posinf=0.0, neginf=0.0)
...
def load_prefix_data(prefix_id: str, paths: PathConfig, logger) -> Optional[Dict[str, Any]]:
    ...
    """
    # Try to import Graph, but make it optional (for attribution t-SNE)
    Graph = None
    try:
        from circuit_tracer.graph import Graph
    except ImportError as e:
        logger.warning(f"  Could not import circuit_tracer.graph: {e}")
        logger.warning("  Attribution space t-SNE will be skipped")
    ...
    # Extract first tokens array and continuation probabilities
    first_tokens = []
    continuation_probs = []
    for ft_data in branches["first_tokens"]:
        token_id = ft_data["token_id"]
        for cont in ft_data["continuations"]:
            first_tokens.append(token_id)
            continuation_probs.append(cont.get("probability", 0.0))
    first_tokens = np.array(first_tokens)
    continuation_probs = np.array(continuation_probs)
    ...
    # Fallback to legacy graph file if needed
    if attributions is None:
        graph_file = attribution_graphs_dir / f"{prefix_id}_graph.pt"
        if Graph is None:
            logger.info(f"  Skipping legacy attribution extraction (circuit_tracer not available)")
        elif not graph_file.exists():
            logger.warning(f"  Graph file not found: {graph_file}")
        else:
            try:
                logger.info(f"  Loading attribution graph...")
                graph = Graph.from_pt(graph_file)
                attributions = extract_attributions(graph, first_tokens, logger)
                logger.info(f"  Extracted attributions: shape {attributions.shape}")
            except Exception as e:
                logger.warning(f"  Failed to extract attributions: {e}")
    ...
    token_id_to_text = {}
    for ft_data in branches["first_tokens"]:
        tid = ft_data["token_id"]
        token_probs[tid] = ft_data.get("first_token_probability", 0.0)
        token_id_to_text[tid] = ft_data.get("token_text", str(tid))
