...
def main():
    ...
    logger.info(f"\nFound {len(clustering_files)} results to visualize")
    ...
    for clustering_file in clustering_files:
        ...
        history = clustering_result.get("history", {})
        if history:
            history_plot = prefix_output_dir / f"{prefix_id}_clustering_history.png"
            plot_clustering_history(history, prefix_id, history_plot)
            logger.info(f"  Saved history plot: {history_plot}")

        # Load semantic graphs
        graphs_file_pt = args.semantic_graphs_dir / f"{prefix_id}_semantic_graphs.pt"
        ...
        if graphs_file_pt.exists():
            ...
            if semantic_graphs:
                heatmap_plot = prefix_output_dir / f"{prefix_id}_semantic_graph_heatmap.png"
            ...
            if token_scores:
                token_plot = prefix_output_dir / f"{prefix_id}_token_scores.png"
                plot_token_scores(
