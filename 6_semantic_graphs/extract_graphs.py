...
def extract_semantic_graphs(
    ...
    # Extract first tokens and probabilities from all continuations
    path_probs_original = []
    first_tokens = []
    ...
    # Compute token scores π_{s,c}
    # π_{s,c} = Σ_{n: s_n=s, c(n)=c} P_n / Σ_{n: s_n=s} P_n

    unique_tokens = list(set(first_tokens))
    token_scores = {}  # token -> {component -> score}

    for token in unique_tokens:
        # Find all samples with this token
        token_indices = [i for i, ft in enumerate(first_tokens) if ft == token]
        token_probs = [path_probs[i] for i in token_indices]
        ...
        # For each component, compute fraction of probability mass
        for c in component_ids:
            ...
            token_scores[token][c] = c_prob / total_prob


    # Compute token-level attribution embeddings G_s
    # G_s = (1 / Σ_{n: s_n=s} P_n) * Σ_{n: s_n=s} P_n * a_n
    logger.info("Computing token-level attribution embeddings G_s...")
    token_attributions = {}
    for s in unique_tokens:
        indices_s = [i for i, t in enumerate(first_tokens) if t == s]
        a_s = attributions_a[indices_s]  # (n_s, E_x)
        P_s = path_probs[indices_s]  # (n_s,)
        total_prob_s = np.sum(P_s)

        if total_prob_s > 0:
            G_s = np.sum(P_s[:, None] * a_s, axis=0) / total_prob_s  # (E_x,)
        else:
            G_s = np.zeros(n_attribution_nodes, dtype=np.float32)

        token_attributions[int(s)] = G_s

    logger.info(f"Computed G_s for {len(token_attributions)} unique tokens")

    # Compute attribution mixture reconstruction
    # G_s ≈ Σ_c π_{s,c} * H_c
    logger.info("Computing attribution mixture reconstruction...")
    token_attributions_reconstructed = {}
    attribution_reconstruction_errors = {}

    for s in unique_tokens:
        if len(component_ids) == 0:
            # No components, reconstruction is zero
            G_s_recon = np.zeros(n_attribution_nodes, dtype=np.float32)
        else:
            # Get π_{s,c} for each component
            pi_s = np.array([token_scores[s][c] for c in component_ids])  # (n_components,)

            # Compute mixture: Σ_c π_{s,c} * H_c
            G_s_recon = np.sum(pi_s[:, None] * graphs_stacked, axis=0)  # (E_x,)

        token_attributions_reconstructed[int(s)] = G_s_recon

        # Compute reconstruction error
        G_s_true = token_attributions[int(s)]
        error = np.linalg.norm(G_s_true - G_s_recon)
        attribution_reconstruction_errors[int(s)] = float(error)

    if len(attribution_reconstruction_errors) > 0:
        errors_array = np.array(list(attribution_reconstruction_errors.values()))
        logger.info(f"Attribution reconstruction errors:")
        logger.info(f"  Mean: {errors_array.mean():.6f}")
        logger.info(f"  Max: {errors_array.max():.6f}")
        logger.info(f"  Min: {errors_array.min():.6f}")
