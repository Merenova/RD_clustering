...
- Extract hidden states for ONLY continuation tokens (first_token + after_continuation)
...
def embed_continuations_for_prefix(
    ...
    Args:
        ...
        continuations: List of continuation texts (first_token + after_continuation)
...
def process_branch_file(
    ...
    - Forward pass on full sequence (prefix + first_token + after_continuation)
    - Extract hidden states for ONLY continuation tokens (first_token + after_continuation)
    ...
    prefix = data["prefix"]
    first_tokens = data["first_tokens"]

    # Collect all continuations across all first tokens
    # continuation = first_token_text + after_continuation_text
    all_continuations = []
    all_metadata = []

    for ft_data in first_tokens:
        first_token_text = ft_data["token_text"]
        continuations = ft_data["continuations"]

        for cont in continuations:
            # Process all continuations (no selection filter)
            # Continuation span: first_token + after_continuation
            continuation = first_token_text + cont["text"]
            all_continuations.append(continuation)
            all_metadata.append({
                "first_token_rank": ft_data["rank"],
                "first_token_id": ft_data["token_id"],
                "first_token_text": first_token_text,
                "first_token_probability": ft_data.get("first_token_probability", 0.0),
    ...
    metadata = {
        ...
        "continuations": all_continuations,  # first_token + after_continuation
        "full_sequences": full_sequences,    # prefix + first_token + after_continuation
