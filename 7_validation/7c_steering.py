...
def compute_branch_log_probs_batch(
    ...
    Returns:
        {branch_id: {"log_P_original": ..., "log_P_first_original": ..., "per_token_log_probs_original": ...}}
    """
    results = {}
    ...
    for i in tqdm(range(0, len(branches), batch_size), desc="Computing branch log_P", leave=False):
        ...
        for j, b in enumerate(batch):
            ...
            # 2. First token log probability
            if cont_ids and cont_start < logits.shape[1]:
                first_token_id = cont_ids[0]
                first_probs = F.log_softmax(logits[j, cont_start, :].float(), dim=0)
                log_P_first_original = float(first_probs[first_token_id].item())
            else:
                log_P_first_original = 0.0

            # 3. Centered logits (already computed)
            per_token_centered_logits, mean_centered_logit = centered_logits_batch[j]

            results[branch_id] = {
                "log_P_original": log_P_original,
                "log_P_first_original": log_P_first_original,
...
def compute_baseline_metadata(
    ...
    Returns:
        metadata: {branch_id: {"log_P_original": ..., "log_P_first_original": ...,
    ...
    for branch in branches:
        ...
        metadata[branch_id] = {
            ...
            "log_P_first_original": branch_log_probs[branch_id]["log_P_first_original"],
