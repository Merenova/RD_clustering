...
def attribute_prefix_to_continuations(
    ...
    *,
    ...
    """
    # Backend dispatch: nnsight backend (used for Gemma3) is delegated to
    # attribute_nnsight_prefix. TL keeps the existing implementation below.
    if getattr(model, "backend", "transformerlens") == "nnsight":
        from .attribute_nnsight_prefix import (
            attribute_prefix_to_continuations as _nn_attr,
        )
        return _nn_attr(
            prefix,
            continuations,
