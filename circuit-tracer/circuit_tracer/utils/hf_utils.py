...
def resolve_transcoder_paths(config: dict) -> dict:
    if "transcoders" in config:
        ...
        # If subfolder is specified, download from that subfolder instead
        subfolder = config.get("subfolder")

        # Construct allow patterns to restrict downloads
        if subfolder:
        ...
        # If subfolder exists, look for layers there
        if subfolder:
...
def generate_gemma_3_config(repo_id: str) -> dict:
    """Generate a config for Gemma 3 scope by auto-selecting the best transcoder variant.

    If repo_id includes additional path components (e.g.,
    "mwhanna/gemma-scope-2-1b-it/transcoder_all/width_16k_l0_small_affine"),
    those are ignored and we extract just the base repo_id.
    """

    # Extract the actual repo_id from the full path if provided
    # (e.g. "mwhanna/gemma-scope-2-1b-it/transcoder_all/width_16k_l0_small_affine" -> "mwhanna/gemma-scope-2-1b-it")
    path_parts = repo_id.split("/")
    if len(path_parts) > 2:
        actual_repo_id = "/".join(path_parts[:2])
    else:
        actual_repo_id = repo_id

    # List available variants and pick the best one
    files = list_repo_files(actual_repo_id)
    ...
    if has_transcoder_all:
        # Pattern for new Gemma Scope 2 structure: transcoder_all/width_*/layer_N.safetensors
        pattern = re.compile(r"transcoder_all/width_(\w+)_l0_(\w+)/layer_(\d+)\.safetensors")
    else:
    ...
    for f in files:
        ...
        if m:
            if has_transcoder_all:
                # For new structure: groups are (width, sparsity, layer)
                width = m.group(1)
                sparsity = m.group(2)
                layer = int(m.group(3))
            else:
                # For old structure: groups are (layer, width, sparsity)
                layer = int(m.group(1))
                width = m.group(2)
                sparsity = m.group(3)
    ...
    for layer, variants in layer_variants.items():
        ...
        transcoders[layer] = f"hf://{actual_repo_id}/{best_variant[2]}"
