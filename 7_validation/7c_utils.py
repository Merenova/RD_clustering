...
def build_branches_from_data(
    ...
    Reconstructs the full branch structure with cluster assignments, handling
    both legacy and new data formats.

    Args:
        branches_data: Loaded branches.json content with keys:
            - prefix: Prefix text
            - prefix_tokens_with_bos: Prefix token IDs including BOS
            - first_tokens: List of first token data with continuations
        assignments: Cluster assignment for each branch (ordered)

    Returns:
        List of branch dictionaries with keys:
            - sequence: Full text sequence (prefix + first_token + continuation)
    ...
    for ft_data in branches_data["first_tokens"]:
        first_token_id = ft_data.get("token_id")
        first_token_text = ft_data.get("token_text", "")

        for cont in ft_data["continuations"]:
            cluster_id = assignments[branch_idx] if branch_idx < len(assignments) else 0

            # Handle both data formats (with/without pre-computed full_token_ids)
            if "full_token_ids" in cont:
                full_token_ids = cont["full_token_ids"]
            elif prefix_tokens_with_bos and first_token_id is not None:
                full_token_ids = prefix_tokens_with_bos + [first_token_id] + cont.get("token_ids", [])
            else:
                full_token_ids = []

            branches.append({
                "sequence": prefix_text + first_token_text + cont.get("text", ""),
