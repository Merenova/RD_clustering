...
def get_top_continuations_per_cluster(
    ...
    """
    For each (token, cluster) pair, find the continuation with highest probability.

    Args:
        branches_data: Branches JSON data with first_tokens and continuations
    ...
    """
    # Track best continuation per (token, cluster)
    ...
    for ft_data in branches_data["first_tokens"]:
        ...
        for cont in ft_data["continuations"]:
            if sample_idx >= len(assignments):
                break

            cluster_id = int(assignments[sample_idx])
            prob = cont["probability"]
            text = cont["text"]

            key = (token_id, cluster_id)
            if key not in best_per_pair or prob > best_per_pair[key]["prob"]:
                best_per_pair[key] = {"text": text, "prob": prob}

            sample_idx += 1

    # Convert to nested dict format
    result: Dict[int, Dict[int, str]] = defaultdict(dict)
    for (token_id, cluster_id), data in best_per_pair.items():
        result[token_id][cluster_id] = data["text"]
